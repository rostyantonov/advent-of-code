package aoc.year2015

import aoc.common.AoCEmptyTest
import aoc.common.IAoCDoubleTest
import org.junit.jupiter.params.provider.Arguments
import java.util.stream.Stream
import kotlin.test.BeforeTest

class Day24Test :
    AoCEmptyTest<Day24, Long>(),
    IAoCDoubleTest<Day24, Long> {
    @BeforeTest
    override fun setupCurrentDay() {
        currentDay = Day24()
    }

    // For example, suppose you have ten packages with weights 1 through 5 and 7 through 11.
    // For this situation, some of the unique first groups, their quantum entanglements,
    // and a way to divide the remaining packages are as follows:
    //  Group 1;             Group 2; Group 3
    //  11 9       (QE= 99); 10 8 2;  7 5 4 3 1
    //  10 9 1     (QE= 90); 11 7 2;  8 5 4 3
    //  10 8 2     (QE=160); 11 9;    7 5 4 3 1
    //  10 7 3     (QE=210); 11 9;    8 5 4 2 1
    //  10 5 4 1   (QE=200); 11 9;    8 7 3 2
    //  10 5 3 2   (QE=300); 11 9;    8 7 4 1
    //  10 4 3 2 1 (QE=240); 11 9;    8 7 5
    //  9 8 3      (QE=216); 11 7 2;  10 5 4 1
    //  9 7 4      (QE=252); 11 8 1;  10 5 3 2
    //  9 5 4 2    (QE=360); 11 8 1;  10 7 3
    //  8 7 5      (QE=280); 11 9;    10 4 3 2 1
    //  8 5 4 3    (QE=480); 11 9;    10 7 2 1
    //  7 5 4 3 1  (QE=420); 11 9;    10 8 2
    override fun partOneInput(): Stream<Arguments> =
        Stream.of(
            Arguments.of(99L, listOf(1, 2, 3, 4, 5, 7, 8, 9, 10, 11)),
        )

    // In the example above, the minimum number of containers was two.
    // There were three ways to use that many containers, and so the answer there would be 3.
    override fun partTwoInput(): Stream<Arguments> =
        Stream.of(
            Arguments.of(44L, listOf(1, 2, 3, 4, 5, 7, 8, 9, 10, 11)),
        )

    override fun realResults(): Stream<Arguments> = Stream.of(Arguments.of(11_266_889_531L, 77_387_711L))
}
