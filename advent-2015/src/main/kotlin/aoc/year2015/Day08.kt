...
class Day08 : AoCFileInput<List<String>, Int>() {
    ...
    override fun processPartOne(): Int {
        val removingReg = "^\"|\"$".toRegex()
        val replacingReg = "\\\\x[a-z0-9]{2}|\\\\\"|\\\\\\\\".toRegex()

        return input.sumOf { line ->
            val resultString = line.replace(removingReg, "").replace(replacingReg, "'")
            line.length - resultString.length
        }
    }
    // result 1 350 for part 1
    ...
    override fun processPartTwo(): Int =
        input.sumOf { line ->
            val resultString = "\"" + line.replace("\\", "\\\\").replace("\"", "\\\"") + "\""
            resultString.length - line.length
        }
    // result 2 085 for part 2
}
