...
import aoc.common.util.decreaseToZero
import aoc.common.util.increase
import aoc.common.util.increaseBy
import aoc.common.util.invert
import aoc.common.util.set
import aoc.year2015.entity.Command.TOGGLE
import aoc.year2015.entity.Command.TURN_OFF
import aoc.year2015.entity.Command.TURN_ON
import aoc.year2015.entity.Instruction
import aoc.year2015.entity.InstructionCompanion
...
class Day06 : AoCFileInput<List<Instruction>, Int>() {
    ...
    override fun processPartOne(): Int {
        val lampsState =
            Array(1000) {
                BooleanArray(1000) { false }
            }

        input.forEach { instruction ->
            when (instruction.command) {
                TURN_ON -> lampsState[instruction.xRange, instruction.yRange] = true
                TURN_OFF -> lampsState[instruction.xRange, instruction.yRange] = false
                TOGGLE -> lampsState.invert(instruction.xRange, instruction.yRange)
            }
        }
        val result =
            lampsState.sumOf { xLine ->
                xLine.count { lamp ->
                    lamp
                }
            }
    ...
    override fun processPartTwo(): Int {
        ...
        input.forEach { instruction ->
            when (instruction.command) {
                TURN_ON -> lampsBrightness.increase(instruction.xRange, instruction.yRange)
                TURN_OFF -> lampsBrightness.decreaseToZero(instruction.xRange, instruction.yRange)
                TOGGLE -> lampsBrightness.increaseBy(instruction.xRange, instruction.yRange, 2)
            }
        }
        val result =
            lampsBrightness.sumOf { xLine ->
                xLine.sumOf { lamp ->
                    lamp
                }
            }
