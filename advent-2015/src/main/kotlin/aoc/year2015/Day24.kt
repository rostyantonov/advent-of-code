...
import aoc.common.util.containsAny
import java.util.LinkedList
import kotlin.properties.Delegates

class Day24 : AoCFileInput<List<Int>, Long>() {
    override val inputFunction
        get() = IntInput::getIntList

    private var groupsList = mutableListOf<LinkedList<Int>>()

    private var groupLimit by Delegates.notNull<Int>()
     ...
     * The first group goes in the passenger compartment of the sleigh, and the second and third go in containers
     * on either side. Only when all three groups weigh exactly the same amount will the sleigh be able to fly.
     ...
     * needs as few packages as possible so that Santa has some legroom left over. It doesn't matter
     * how many packages are in either of the other two groups, so long as all of the groups weigh the same.
     *
     * Furthermore, Santa tells you, if there are multiple ways to arrange the packages such that the fewest
     * possible are in the first group, you need to choose the way where the first group has the smallest quantum
     * entanglement to reduce the chance of any "complications". The quantum entanglement of a group of packages
     * is the product of their weights, that is, the value you get when you multiply their weights together.
     ...
     * some of the unique first groups, their quantum entanglements, and a way to divide
     ...
     * 11 9, in the passenger compartment gives Santa the most legroom and wins. In this situation,
     ...
     */
    override fun processPartOne(): Long = findSolution(3)
    // result 11 266 889 531 for part 1
     ...
     * 111 4, 10 5, and 8 7. Of these, 11 4 has the lowest quantum entanglement, and so it is selected.
     *
     * Now, what is the quantum entanglement of the first group of packages in the ideal configuration?
     */
    override fun processPartTwo(): Long = findSolution(4)
    ...
    private fun findSolution(numOfBags: Int): Long {
        ...
        groupsList.sortBy { bag -> bag.size }
        var result = 0L
        groupsList.forEachIndexed { index, ints ->
            val quantum = getQuantum(ints)
            if (result > 0 || quantum < 0) {
                return@forEachIndexed
            }
            for (secondIndex in index..<groupsList.size) {
                if (!groupsList[secondIndex].containsAny(ints)) {
                    result = quantum
                    return@forEachIndexed
                }
            }
    ...
    private fun findAllPossibleGroups(
        ...
        start: Int = 0,
        currentGroup: LinkedList<Int> = LinkedList(),
    ) {
        for (index in start..<input.size) {
            ...
            if (currentSum == groupLimit) {
                ...
                findAllPossibleGroups(groupsList, index + 1, currentGroup)
                currentGroup.removeLast()
            }
        }
    }

    private fun getQuantum(bag: LinkedList<Int>): Long {
        var quantum = 1L
        bag.forEach { quantum *= it }
        return quantum
    }
}
