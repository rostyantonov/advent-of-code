...
import aoc.year2015.entity.Reindeer
import aoc.year2015.entity.ReindeerCompanion

class Day14 : AoCFileInput<List<Reindeer>, Int>() {
    private val ticks = 2503
     ...
     * (poor Dancer has only gotten 1056 km by that point). So, in this situation,
     ...
     */
    override fun processPartOne(): Int = functionPartOne()
    // result 2 655 for part 1

    internal fun functionPartOne(ticks: Int = this.ticks): Int {
        val reindeerList = input.clone()
        iterateTicks(reindeerList, ticks)
        return reindeerList.maxOf { it.distanceTraveled }
    }
     ...
     *
     * After the 1000th second, Dancer has accumulated  v points, while poor Comet, our old champion,
     ...
     */
    override fun processPartTwo(): Int = functionPartTwo()
    // result 1 059 for part 2

    internal fun functionPartTwo(ticks: Int = this.ticks): Int {
        val reindeerList = input.clone()
        iterateTicks(reindeerList, ticks)
        return reindeerList.maxOf { it.points }
    }

    private fun iterateTicks(
        reindeerList: List<Reindeer>,
        ticks: Int,
    ) {
        repeat(ticks) {
            ...
            val maxValue =
                reindeerList.maxOf { reindeer ->
                    reindeer.distanceTraveled
                }
            reindeerList.forEach { reindeer ->
                if (reindeer.distanceTraveled == maxValue) {
                    reindeer.addPoint()
                }
            }
        }
