    ...
    inner class StructureVisitor : KSVisitorVoid() {
        ...
        private fun generateLineBasedParameterMappings(parameters: List<com.google.devtools.ksp.symbol.KSValueParameter>): String =
            parameters.joinToString(",\n") { param ->
                ...
                // Special handling for 'range' field name - maps to collection.range
                if (name == "range" && typeString == "IntRange") {
                    return@joinToString "            $name = collection.range"
                }
                ...
                if (parameters.isEmpty()) {
                    ...
                    val paramsList =
                        parameters.mapNotNull { param ->
                            ...
                            val hasDefaultValue = param.hasDefault
                            
                            // Check for @FieldConverter annotation
                            val converterClass =
                            ...
                            val getter =
                                if (converterClass != null) {
                                    // Use the FieldConverter
                                    "$converterClass.convert(collection, \"$paramName\")"
