...
internal class ParameterMappings(
     ...
     * produces its value.
     */
    fun getterExpression(param: KSValueParameter): String {
        ...
        if (enumDeclarationOf(param) != null) {
            ...
            return "BaseEntity.$getter<$typeString>(collection, \"$name\")"
        }
     ...
     *
     * The token is the subclass simple name uppercased (Hlf -> HLF), which matches the runtime
     * discriminator because the generated `create` uppercases the group value too.
     */
    fun discriminatorTokens(subclasses: List<KSClassDeclaration>): Map<KSClassDeclaration, String> =
        subclasses.associateWith { subclass -> subclass.simpleName.asString().uppercase() }
