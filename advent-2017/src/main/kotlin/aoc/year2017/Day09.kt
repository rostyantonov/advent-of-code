...
import aoc.common.entity.CharConstants.CLOSING_CURLY_BRACKET
import aoc.common.entity.CharConstants.OPENING_CURLY_BRACKET
import aoc.common.input.AoCFileInput
import aoc.common.input.StringInput

class Day09 : AoCFileInput<String, Int>() {
     ...
     * a } only closes the most-recently-opened unclosed group - that is, they are nestable. Your puzzle input
     * represents a single, large group which itself contains many smaller ones.
     *
    ...
    override fun processPartOne(): Int =
        input
            .replace(Regex("!.{1}"), "")
            .replace(Regex("<.*?>"), "")
            .foldIndexed(0 to 0) { index, (score, depth), char ->
                when (char) {
                    OPENING_CURLY_BRACKET -> score to depth + 1
                    CLOSING_CURLY_BRACKET -> score + depth to depth - 1
                    else -> score to depth
                }
            }.first
    // result 11 089 for part 1
    ...
    override fun processPartTwo(): Int =
        Regex("<.*?>")
            .findAll(input.replace(Regex("!.{1}"), ""))
            .sumOf { it.value.length - 2 }
    // result 5 288 for part 2
}
