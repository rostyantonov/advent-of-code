...
class Day12 : AoCFileInput<List<List<Int>>, Int>() {
    override val inputFunction
        get() = IntInput::getIntOfIntList
     ...
     *
     * Therefore, a total of 6 programs are in this group; all but program 1, which has a pipe that connects it
     * to itself.
     *
     * How many programs are in the group that contains program ID 0?
     */
    override fun processPartOne(): Int {
        val programs = input.associate { it.first() to it.drop(1) }
        val group = mutableSetOf<Int>()

        findGroup(0, group, programs)
        return group.size
    }
    // result 378 for part 1
    ...
    override fun processPartTwo(): Int {
        val programs = input.associate { it.first() to it.drop(1) }.toMutableMap()
        var groupCounter = 0
        while (programs.isNotEmpty()) {
            val group = mutableSetOf<Int>()
            findGroup(programs.keys.first(), group, programs)
            group.forEach { programs.remove(it) }
            groupCounter++
        }
        return groupCounter
    }
    // result 204 for part 2

    private fun findGroup(
        programId: Int,
        group: MutableSet<Int>,
        programs: Map<Int, List<Int>>,
    ) {
        if (programId !in group) {
            group.add(programId)
            programs[programId]!!.forEach { findGroup(it, group, programs) }
        }
    }
}
