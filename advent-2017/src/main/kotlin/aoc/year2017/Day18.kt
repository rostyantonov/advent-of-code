...
import aoc.year2017.entity.PairedDuetComputer
import aoc.year2017.entity.SoloDuetComputer
...
class Day18 : AoCFileInput<List<AsmInstruction>, Int>() {
        ...
        get() =
            StructuredMultiInput
                .of(
                    regexArray =
                        arrayOf(
                            AsmInstructionPatterns.SEND,
                            AsmInstructionPatterns.ASSIGN,
                            AsmInstructionPatterns.ADD,
                            AsmInstructionPatterns.MULTIPLY,
                            AsmInstructionPatterns.MODULO,
                            AsmInstructionPatterns.RECEIVE,
                            AsmInstructionPatterns.JUMP_IF_POSITIVE,
                        ),
     ...
     * sound card with it, you decide to run the code yourself. Unfortunately, you don't see any documentation,
     ...
     *          and so on.)
     *
     * Many of the instructions can take either a register (a single letter) or a number. The value of a register
     * is the integer it contains; the value of a number is that number.
     *
    ...
    override fun processPartOne(): Int =
        SoloDuetComputer(input)
            .apply { run() }
            .recovered
            ?.toInt()
            ?: NOTHING_RECOVERED
    // result 8 600 for part 1

    /**
     * As you congratulate yourself for a job well done, you notice that the documentation has been on the back
     * of the tablet this entire time. While you actually got most of the instructions correct, there are a few
     * key differences. This assembly code isn't about sound at all - it's meant to be run twice at the same time.
     *
     * Each running copy of the program has its own set of registers and follows the code independently - in
     * fact, the programs don't even necessarily run at the same speed. To coordinate, they use the send (snd)
     ...
     *          ready to receive them. Each program has its own message queue, so a program can never receive
     *          a message it sent.
     ...
     *
     * Each program also has its own program ID (one 0 and the other 1); the register p should begin with this
     * value.
     *
     * For example:
     *
     ...
     * it in b, and then each receives the program ID of the other program (program 0 receives 1; program 1
     * receives 0) and stores it in c. Each program now sees a different value in its own copy of register c.
     *
     ...
     */
    override fun processPartTwo(): Int = PairedDuetComputer.runDuet(input).toInt()
    // result 7 239 for part 2

    companion object {
        /** Returned when the program halts without ever recovering a frequency. */
        private const val NOTHING_RECOVERED = -1
    }
}
