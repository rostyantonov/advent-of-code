...
class Day04 : AoCFileInput<List<List<String>>, Int>() {
    override val inputFunction
        get() = StringInput::getStringOfStringList
     ...
     */
    override fun processPartOne(): Int = input.count { it.size == it.distinct().size }
    // result 466 for part 1
    ...
    override fun processPartTwo(): Int =
        input.count {
            it.forEachIndexed { index, string ->
                it.forEachIndexed { innerIndex, innerString ->
                    if (innerIndex <= index) return@forEachIndexed
                    if (string.length == innerString.length &&
                        string.toCharArray().sorted() == innerString.toCharArray().sorted()
                    ) {
                        return@count false
                    }
                }
