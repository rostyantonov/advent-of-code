...
class Day13 : AoCFileInput<List<Pair<Int, Int>>, Int>() {
    override val inputFunction
        get() = IntInput::getPairOfInt
     ...
     * after that (with range 2), a third layer which begins at depth 4 (with range 4), and a fourth layer which
     ...
     *                      [S]     [S]
     *
     * Your plan is to hitch a ride on a packet about to move through the firewall. The packet will travel along
     * the top of each layer, and it moves at one layer per picosecond. Each picosecond, the packet moves one
     * layer forward (its first move takes it into layer 0), and then the scanners move one step. If there is
     * a scanner at the top of the layer as your packet enters it, you are caught. (If a scanner moves into the
     * top of its layer while you are there, you are not caught: it doesn't have time to notice you before you
     * leave.) If you were to do this in the configuration above, marking your current position with parentheses,
     ...
     *
     * The severity of getting caught on a layer is equal to its depth multiplied by its range. (Ignore layers
     * in which you do not get caught.) The severity of the whole trip is the sum of these values. In the
    ...
    override fun processPartOne(): Int {
        val depth = input.maxOf { it.first } // get max depth
        val scanners = input.associate { it.first to it.second } // create map of scanners
        var severity = 0
        (0..depth).forEach {
            val range = scanners[it] ?: 0
            if (range > 0 && it % (2 * (range - 1)) == 0) {
                severity += it * range
            }
        }
        return severity
    }
    // result 1 624 for part 1
    ...
    override fun processPartTwo(): Int {
        ...
        var step = 0 // Delay in picoseconds

        while (true) {
            var caught = false

            // check if we will be caught at any layer with the current delay
            for (it in 0..depth) {
                val range = scanners[it] ?: 0
                // If we already faced scann no need to continue
                if (range > 0 && (step + it) % (2 * (range - 1)) == 0) {
                    caught = true
                    break
                }
            }
