...
class Day10 : AoCFileInput<List<String>, String>() {
     ...
     * anyway. You make a mental note to remind the Elves later not to invent their own cryptographic functions.
     *
     ...
     * span a half-twist to reverse the order of the marks within it. After doing this many times, the order of
     ...
     * If the current position moves past the end of the list, it wraps around to the front. Lengths larger than
     ...
     *     The current position moves forward by the length (1) plus the skip size (2): 4 [3] 0 1 2. The skip size
     *            increases to 3.
     *
     ...
     * lengths in your puzzle input. Once this process is complete, what is the result of multiplying the first
     * two numbers in the list?
     */
    override fun processPartOne(): String = solvePartOne(size).toString()
    // result 11 413 for part 1

    fun solvePartOne(size: Int): Int =
        KnotHash(size)
            .apply { rotate(IntInput.getFirstLineIntList(input)) }
            .multiplyFirstTwo()
     ...
     * arbitrary ASCII strings, and it also ensures that your input lengths are never larger than 255. For example,
     * if you are given 1,2,3, you should convert it to the ASCII codes for each character: 49,44,50,44,51.
     *
     ...
     * For example, if the previous example was your first round, you would start your second round with the same
     * length sequence (3, 4, 1, 5, 17, 31, 73, 47, 23, now assuming they came from ASCII codes and include the
     ...
     *
     * Finally, the standard way to represent a Knot Hash is as a single hexadecimal string; the final output is
     * the dense hash in hexadecimal notation. Because each number in your dense hash will be between 0 and 255
     * (inclusive), always represent each number as two hexadecimal digits (including a leading zero as necessary).
     ...
     */
    override fun processPartTwo(): String = KnotHash.of(input.first())
    // result 7adfd64c2a03a4968cf708d1b7fd418d part 2
}
