...
import aoc.year2017.entity.Tower
import aoc.year2017.entity.TowerCompanion
...
class Day07 : AoCFileInput<List<Tower>, String>() {
     ...
     * the bottom program?
     */
    override fun processPartOne(): String = findBottomTower().name
    // result eugwuhl for part 1
    ...
    override fun processPartTwo(): String {
        val bottomTower = findBottomTower()
        val towers = input.associateBy { it.name }

        fun calculateWeight(tower: Tower): Int {
            tower.calculatedWeight =
                if (tower.itemNames.isNullOrEmpty()) {
                    tower.weight
                } else {
                    tower.weight + tower.itemNames!!.sumOf { calculateWeight(towers[it]!!) }
                }
            return tower.calculatedWeight
        }

        calculateWeight(bottomTower)

        val groups = input.groupBy { it.calculatedWeight }
        val once = groups.filter { it.value.size == 1 }.toList().sortedBy { it.first }
        val child = once.first().second.first()
        val parent = once[1].second.first()
        val diff =
            child.calculatedWeight - (towers[parent.itemNames!!.first { it != child.name }]?.calculatedWeight ?: 0)
        val result = child.weight - diff

        return result.toString()
    }
    // result 420 for part 2

    private fun findBottomTower(): Tower {
        val subTowers = input.flatMap { it.itemNames ?: emptyList() }
        return input.first { it.name !in subTowers }
    }
}
