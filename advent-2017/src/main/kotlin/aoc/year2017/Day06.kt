...
class Day06 : AoCFileInput<List<Int>, Int>() {
    override val inputFunction
        get() = IntInput::getFirstLineIntList
     ...
     *          and so on, the 7 blocks are spread out over the memory banks. The fourth, first, and second banks
     *          get two blocks each, and the third bank gets one back. The final result looks like this: 2 4 1 2.
     ...
     *          The first bank wins the tie, and its three blocks are distributed evenly over the other
     ...
     */
    override fun processPartOne(): Int = solveBothParts(true)
    // result 5 042 for part 1
     ...
     */
    override fun processPartTwo(): Int = solveBothParts(false)
    // result 1 086 part 2

    private fun solveBothParts(isFirst: Boolean): Int {
        var steps = 0
        val instructions = input.toMutableList()
        val seenStates = mutableSetOf<List<Int>>()
        while (instructions !in seenStates) {
            seenStates.add(instructions.toList())
            val maxIndex = instructions.withIndex().maxByOrNull { it.value }!!.index
            val blocksToRedistribute = instructions[maxIndex]
            instructions[maxIndex] = 0
            var currentIndex = (maxIndex + 1) % instructions.size
        ...
        return seenStates.drop(seenStates.indexOf(instructions)).size
    }
}
