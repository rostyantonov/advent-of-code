...
import aoc.common.grid.GridMap
import aoc.common.input.AoCFileInput
import aoc.common.input.IntInput

class Day03 : AoCFileInput<Int, Int>() {
    override var rawInput = listOf("312051")
    ...
    override fun processPartOne(): Int {
        ...
        val minDistanceToMidpoint = midPoints.minOf { kotlin.math.abs(input - it) }
        return layer + minDistanceToMidpoint
    }
    // result 430 for part 1
     ...
     */
    override fun processPartTwo(): Int = solvePartTwo()
    // result 312 453 for part 2

    private fun solvePartTwo(): Int {
        val walker = FacingWalker(facingDirection = DOWN)
        val gridMap = GridMap<Int>().apply { setValue(walker.position, 1) }

        while (true) {
            when (walker.facingDirection) {
                DOWN -> if (gridMap.getValue(walker.position.getRight()) == null) walker.facingDirection = RIGHT
                RIGHT -> if (gridMap.getValue(walker.position.getUp()) == null) walker.facingDirection = UP
                UP -> if (gridMap.getValue(walker.position.getLeft()) == null) walker.facingDirection = LEFT
                LEFT -> if (gridMap.getValue(walker.position.getDown()) == null) walker.facingDirection = DOWN
            }
            walker.moveForward()
            gridMap.getNeighbours(walker.position).sum().let { sum ->
                if (sum > input) return sum
                gridMap.setValue(walker.position, sum)
            }
        }
    }
}
