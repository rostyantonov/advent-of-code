...
class Day02 : AoCFileInput<List<List<Int>>, Int>() {
     ...
     */
    override fun processPartOne(): Int = input.sumOf { it.max() - it.min() }
    // result 41 887 for part 1
    ...
    override fun processPartTwo(): Int =
        input.sumOf { row ->
            row
                .asSequence()
                .flatMap { a ->
                    row.asSequence().mapNotNull { b ->
                        if (a != b && a % b == 0) a / b else null
                    }
                }.first()
        }
    // result 226 for part 2
}
