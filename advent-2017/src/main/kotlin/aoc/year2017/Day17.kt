...
class Day17 : AoCFileInput<Int, Int>() {
    override var rawInput = listOf("371")
     ...
     * spinlock. This spinlock isn't just consuming computing power, but memory, too; vast, digital mountains
     * are being ripped from the ground and consumed by the vortex.
     *
     * If you don't move quickly, fixing that printer will be the least of your problems.
     *
     * This spinlock's algorithm is simple but efficient, quickly consuming everything in its path. It starts
     * with a circular buffer containing only the value 0, which it marks as the current position. It then
     * steps forward through the circular buffer some number of steps (your puzzle input) before inserting
     * the first new value, 1, after the value it stopped on. The inserted value becomes the current position.
     * Then, it steps forward from there the same number of steps, and wherever it stops, inserts after it the
     ...
     *     0 (2) 1: the spinlock steps forward three times (0, 1, 0), and then inserts the second value, 2,
     *             after it. 2 becomes the current position.
     *     0  2 (3) 1: the spinlock steps forward three times (1, 0, 2), and then inserts the third value, 3,
     *             after it. 3 becomes the current position.
     *
     * And so on:
     *
     *     0  2 (4) 3  1
    ...
    override fun processPartOne(): Int {
        ...
        for (i in 1..2017) {
            // Find the position to insert: move forward by input steps
            // and take modulo the current buffer size
            currentPosition = (currentPosition + input) % buffer.size

            // Insert right after the current position
            val insertPosition = currentPosition + 1
            buffer.add(insertPosition, i)

            // The new number becomes the current position
            currentPosition = insertPosition
        }

        // Ищем, какое число идет сразу после 2017
        val indexOf2017 = buffer.indexOf(2017)
        return buffer[(indexOf2017 + 1) % buffer.size]
    }
    // result 1 311 for part 1
    ...
    override fun processPartTwo(): Int {
        ...
        for (i in 1..50_000_000) {
            ...
            // If the inserted number will be inserted immediately after 0, remember it
