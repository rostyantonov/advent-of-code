...
class Day01 : AoCFileInput<List<Int>, Int>() {
    override val inputFunction
        get() = IntInput::getCharsAsIntList
     ...
     * midnight. "We have a big problem," she says; "there must be almost fifty bugs in this system, but nothing
     * else can print The List. Stand in this square, quick! There's no time to explain; if you can convince them
     * to pay you in stars, you'll be able to--" She pulls a lever and the world goes blurry.
     *
     * When your eyes can focus again, everything seems a lot more pixelated than before. She must have sent you
     * inside the computer! You check the system clock: 25 milliseconds until midnight. With that much time, you
     ...
     */
    override fun processPartOne(): Int = matcher(1)
    // result 1 171 for part 1

    /**
     * You notice a progress bar that jumps to 50% completion. Apparently, the door isn't yet satisfied, but it did
     ...
     */
    override fun processPartTwo(): Int = matcher(input.size / 2)
    // result 1 024 for part 2

    private fun matcher(step: Int): Int =
        input.foldIndexed(0) { index, acc, i ->
            val nextIndex = (index + step) % input.size
            if (i == input[nextIndex]) {
                acc + i
            } else {
                acc
            }
