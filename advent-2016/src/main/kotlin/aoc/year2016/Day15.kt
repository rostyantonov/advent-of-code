...
import aoc.year2016.entity.Disc
import aoc.year2016.entity.DiscCompanion
...
class Day15 : AoCFileInput<List<Disc>, Int>() {
     ...
     * enclosure and seems to involve a set of identical spherical capsules that are carried to the top and allowed
     * to bounce through the maze of spinning pieces.
     *
     ...
     * of positions at which they stop. You decide to call the position with the slot 0, and count up for each
     ...
     *
     * However, your situation has more than two discs; you've noted their positions in your puzzle input. What is
     * the first time you can press the button to get a capsule?
     */
    override fun processPartOne(): Int = solve()
    // result 203 660 for part 1
     ...
     */
    override fun processPartTwo(): Int = solve(input + Disc(input.size + 1, 11, 0))
    // result 2 408 135 for part 2

    private fun solve(discs: List<Disc> = input): Int =
        generateSequence(
            0,
            Int::inc,
        ).first { time -> discs.all { it.isOpenAt(time) } }
}
