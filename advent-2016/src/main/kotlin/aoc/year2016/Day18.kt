...
import aoc.common.entity.CharConstants.CHAR_CARET as TRAP_TILE
import aoc.common.entity.CharConstants.CHAR_DOT as SAFE_TILE

/**
 * Day 18: Like a Rogue
 */
class Day18 : AoCFileInput<String, Int>() {
     ...
     * pressure plates for traps, and the trap you just triggered has run out of... whatever it tried to do
     ...
     * are arranged into rows that are all the same width; you take note of the safe tiles (.) and traps (^)
     ...
     * and to either side of that position, in the previous row. (If either side is off either end of the row,
     * it counts as "safe" because there isn't a trap embedded in the wall.)
     *
     * For example, suppose you know the first row (with tiles marked by letters) and want to determine the
     * next row (with tiles marked by numbers):
     *
     *     ABCDE
     *     12345
     *
     * The type of tile 2 is based on the types of tiles A, B, and C; the type of tile 5 is based on tiles D,
     * E, and an imaginary "safe" tile. Let's call these three tiles from the previous row the left, center,
     ...
     *
     * Then, starting with the row ..^^., you can determine the next row by applying those rules to each
     * new tile:
     *
     ...
     *     previous row. Because all of the trap rules require a trap in at least one of the previous three
     ...
     * next row, using the same rules, and get ^^..^. After determining two new rows, our map looks like this:
     *
     *     ..^^.
     *     .^^^^
     *     ^^..^
     ...
     */
    override fun processPartOne(): Int = countSafeTiles(40)
    // result 1951 for part 1

    /**
     * How many safe tiles are there in a total of 400000 rows?
     */
    override fun processPartTwo(): Int = countSafeTiles(400_000)
    // result 20 002 936 for part 2

    /**
     * Count safe tiles across the specified number of rows.
     */
    internal fun countSafeTiles(rows: Int): Int =
        generateSequence(input) { row -> generateNextRow(row) }
            .take(rows)
            .sumOf { row -> row.count { it == SAFE_TILE } }

    /**
     * Generate the next row based on trap rules.
     * A tile is a trap if left XOR right (simplified from the 4 patterns).
     */
    private fun generateNextRow(row: String): String =
        buildString(row.length) {
            row.indices.forEach { index ->
                val left = row.getOrNull(index - 1) ?: SAFE_TILE
                val right = row.getOrNull(index + 1) ?: SAFE_TILE

                // Trap if left and right differ
                append(if (left != right) TRAP_TILE else SAFE_TILE)
            }
        }
}
