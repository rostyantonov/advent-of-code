    ...
    private fun doComputations(registers: MutableMap<String, Int>): Int {
        ...
        fun detectMultiplication(pos: Int): Boolean {
            ...
            if (i0 is AsmInstruction.Cpy &&
                ...
                AsmComputer.isRegister(i1.valueOrRegister) &&
                AsmComputer.isRegister(i1.register2) &&
        ...
        fun detectAddition(pos: Int): Boolean {
            ...
            if (i0 is AsmInstruction.Cpy &&
                ...
                if (innerCountReg == i3.register &&
                    ...
                    val outerCount = AsmComputer.getValue(i0.valueOrRegister, registers)
                    val innerCount = AsmComputer.getValue(i1.valueOrRegister, registers)
        ...
        while (position in instructions.indices) {
            ...
            if (ins is AsmInstruction.Tgl) {
                val offset = AsmComputer.getValue(ins.register, registers)
                ...
                val jump =
                    when (ins) {
                        is AsmInstruction.Cpy -> {
                            ...
                            if (AsmComputer.isRegister(ins.register2)) ins.execute(registers) else 1
                        }

                        is AsmInstruction.Inc -> {
                            // Only valid if target is a register
                            if (AsmComputer.isRegister(ins.register)) ins.execute(registers) else 1
                        }
