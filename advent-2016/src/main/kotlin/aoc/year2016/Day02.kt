...
class Day02 : AoCFileInput<List<String>, String>() {
     ...
     */
    override fun processPartOne(): String {
        val keyPad: Array<Array<Char?>> =
            arrayOf(
                arrayOf('1', '2', '3'),
                arrayOf('4', '5', '6'),
                arrayOf('7', '8', '9'),
        ...
        return iterateKeyPad(keyPad, startPosition)
    }
    // result 35749 for part 1

    /**
     * You finally arrive at the bathroom (it's a several minute walk from the lobby so visitors can behold
     * the many fancy conference rooms and water coolers on this floor) and go to punch in the code.
     * Much to your bladder's dismay, the keypad is not at all like you imagined it. Instead, you are confronted
    ...
    // result 9365C for part 2

    private fun iterateKeyPad(
        ...
        input.forEach { line ->
            line.forEach { char ->
                val nextPosition =
                    when (char) {
                        UP_CHAR -> position.getUp()
                        DOWN_CHAR -> position.getDown()
                        LEFT_CHAR -> position.getLeft()
                        RIGHT_CHAR -> position.getRight()
                        else -> throw UnsupportedTypeException("Unsupported direction provided")
                    }
                if (keyPad[nextPosition] != null) {
