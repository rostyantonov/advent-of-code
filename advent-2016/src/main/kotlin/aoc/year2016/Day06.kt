...
class Day06 : AoCFileInput<List<String>, String>() {
     ...
     */
    override fun processPartOne(): String = decodeMessage(mostCommon = true)
    // result xdkzukcf for part 1
     ...
     * noise, you can look at the letter distributions in each column and choose the least common letter
     ...
     */
    override fun processPartTwo(): String = decodeMessage(mostCommon = false)
    // result cevsgyvd for part 2

    private fun decodeMessage(mostCommon: Boolean): String {
        val messageLength = input.first().length
        return buildString(messageLength) {
            for (columnIndex in 0 until messageLength) {
                val charFrequencies =
                    input
                        .map { it[columnIndex] }
                        .groupingBy { it }
                ...
                val selectedChar =
                    if (mostCommon) {
                        charFrequencies.maxBy { it.value }.key
                    } else {
                        charFrequencies.minBy { it.value }.key
                    }
