...
object BaseEntity {
    ...
    /**
     * The group value read as a [T] constant, normalised the way input text tends to differ from
     * Kotlin naming: case is ignored and spaces stand in for underscores ("turn on" -> TURN_ON).
     *
     * Mirrors `aoc.common.util.safeValue`, which cannot be reused here because `common` depends on
    ...
    inline fun <reified T : Enum<T>> getAsNullableEnum(
        ...
        val raw = collection[name]?.value ?: return null
        val normalised = raw.replace(' ', '_').uppercase()
